...
sds sdsempty(void);											//创建一个只包含空白字符串""的sds O(N)
...
sds sdscatlen(sds s, const void *t, size_t len);			//对sds的buf按给定长度扩展 并将给定字符串追加到末尾 
															//如果给定字符串t长度大于len 只追加len长度其余截断 
															//如果给定字符串t长度小于len 多余的长度补'\0' O(N)
